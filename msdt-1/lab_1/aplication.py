...
from keras.api.preprocessing.sequence import TimeseriesGenerator
...
# Styles for selected and unselected tabs
tab_selected_style = {
...
# Convert sequence data to supervised learning format
...
# Function to plot training and test data with predictions
def plot_train_test_graph(stock, model, test_generator,
                          close_train, close_test, date_train, date_test):
...
# Function for predicting future stock prices
...
# Function to predict the future dates for the predictions
...
def plot_future_prediction(model, test_generator, close_train,
                           close_test, df, forecast_dates, forecast):
    prediction = model.predict_generator(test_generator)
    close_train.reshape((-1))
    close_test.reshape((-1))
...
app.layout = html.Div([
    ...
    html.H2("Created By - Rishabh Panesar",
            style={"textAlign": "center", "margin_top": "8px"}),
    dcc.Tabs(id="tabs", children=[
        dcc.Tab(label="Some Basic Information", children=[
            html.Div([
                html.P("This project is made just for educational purpose. "
                       "All the predictions made by the Machine Learning "
                       "Model are entirely probabilistic based.",
                ...
                html.Ol(children=[
                    ...
                    html.Li("First Plot is the plot obtained from testing the model against past data"),
        ...
        dcc.Tab(label="See the model in Action", children=[
            ...
            html.H1("Future Price Prediction", style={"textAlign": "center", "margin-top": "20px"}),
            html.Div([
...
# Function for empty graph layout
...
# Define the callback for updating graphs
@app.callback(
...
def update_graph(stock_name):
    try:
        df, close_data, info = download_and_process_data(stock_name)
    except:
    ...
    try:
        ...
        figure_1, r2_score_value = plot_train_test_graph(stock_name, lstm_model,
        ...
        figure_2 = plot_future_prediction(lstm_model, test_generator,
                                          close_train, close_test, df, forecast_dates, forecast)
        r2_score_value = "R2 Score : {}".format(r2_score_value)
        return figure_1, figure_2, r2_score_value, info.info["longBusinessSummary"]
    except:
        empty = return_empty_graph()
        return empty, empty, "No R2 Score to display", "No Asset Queried or Selected"
